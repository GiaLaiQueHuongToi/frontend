...
export const ImageWithFallback = ({
    ...
    useEffect(() => {
        if (src.includes('pollinations') && loadDelay > 0) {
            console.log(
                `⏳ Delaying Pollinations image load for segment ${segmentId} by ${loadDelay}ms`
            );
            const delayTimeout = setTimeout(() => {
                console.log(
                    `🚀 Starting delayed load for segment ${segmentId}`
                );
                setImgSrc(src);
    ...
    const handleError = () => {
        console.log(
            `❌ Image error for segment ${segmentId}, fallback level: ${fallbackLevel}`
        );
        ...
        if (fallbackLevel === 0) {
            ...
            if (imgSrc.includes('pollinations')) {
                ...
                setImgSrc(
                    `${baseUrl}?width=1920&height=1080&seed=${newSeed}&nologo=true`
                );
                ...
                setFallbackLevel(1);
                const newImageId =
                    100 + segmentId * 20 + Math.floor(Math.random() * 50);
                setImgSrc(
                    `https://picsum.photos/1920/1080?random=${newImageId}`
                );
    ...
    const handleLoad = () => {
        setIsLoading(false);
        console.log(
            `✓ Image loaded successfully for segment ${segmentId}: ${imgSrc}`
        );
    };
            ...
            {(!shouldStartLoading || isLoading) && (
                <div className='absolute inset-0 flex items-center justify-center bg-gray-100 z-10'>
                    <div className='text-center'>
                        <div className='animate-spin w-6 h-6 border-2 border-purple-500 border-t-transparent rounded-full mb-2 text-center'></div>
                        <div className='text-xs text-gray-500'>
                            {!shouldStartLoading
                                ? `Waiting... (${Math.floor(loadDelay / 1000)}s)`
                                : 'Loading...'}
                        </div>
                    </div>
                </div>
