...
export function VoiceGenerationStep({ imageState, state, onUpdateState, onSegmentsChange, initialSegments, previousStep }: VoiceGenerationStepProps) {
    ...
    const generateAllAudio = async () => {
        if (!imageState.generatedImages || imageState.generatedImages.length === 0) return;
            ...
            for (const segment of sourceSegments) {
                if (segment.scriptText) {
                    try {
                        ...
                        console.log(`Generating audio for segment ${segment.segmentId}: ${segment.scriptText}`);

                        const audioUrl = await audioService.generateAudio(audioRequest);

                        // Get audio duration (approximate based on text length)
                        const duration = Math.max(3, segment.scriptText.length * 0.08); // ~80ms per character, minimum 3 seconds
                        ...
                        console.warn(`Failed to generate audio for segment ${segment.segmentId}:`, audioError);
                        ...
                        newSegments.push({
                            ...
                            duration: Math.max(3, segment.scriptText.length * 0.08)
                        });
                    }
                }
            }
    ...
    const generateSingleAudio = async (segmentId: number) => {
        ...
        try {
            ...
            console.log(`Generating audio for segment ${segmentId}: ${segment.scriptText}`);
            ...
            <div className='space-y-4'>
                <div className='flex gap-4 flex-wrap'>
                    ...
                    >
                        ...
                        {isGeneratingAudio
                            ? 'Generating Audio...'
                            : segments.length > 0 && segments.some(s => s.audioUrl)
                ...
                {isGeneratingAudio && (
                    <div className='flex items-center gap-2'>
                        ...
                        <span className='text-sm text-gray-600'>Generating audio for all segments...</span>
                    </div>
                )}
            </div>
            ...
            {segments.length > 0 && (
                <div className='space-y-4'>
                    ...
                    {segments.map((segment, index) => (
                        <div key={`${segment.segmentId}-${audioGenerationKey}`} className='p-4 bg-gray-50 rounded-lg border'>
                            <div className='flex items-start justify-between mb-3'>
                                <h4 className='font-medium'>Segment {segment.segmentId}</h4>
                                <Button
                                    size='sm'
                                    variant='outline'
                                    onClick={() => generateSingleAudio(segment.segmentId)}
                                    disabled={
                                ...
                                >
                                    ...
                                    {generatingSegmentId === segment.segmentId ? 'Generating...' : 'Generate Audio'}
                                </Button>
                            </div>
                            ...
                            <div className='flex items-start gap-4'>
                                ...
                                <div className='flex-1'>
                                    ...
                                    <div className='mb-3'>
                                        <Label className='text-xs text-gray-500'>Script Text:</Label>
                                        <Textarea
                                            value={segment.scriptText}
                                            onChange={(e) => updateSegmentScript(segment.segmentId, e.target.value)}
                                    ...
                                    <div className='space-y-2'>
                                        {segment.audioUrl && (
                                            <audio key={segment.audioUrl} controls className='w-full'>
                                                <source src={segment.audioUrl} type='audio/mp3' />
                                            </audio>
                                        )}

                                        {!segment.audioUrl && generatingSegmentId !== segment.segmentId && (
                                            <p className='text-xs text-gray-400 italic'>No audio generated</p>
                                        )}
                                    </div>
                                </div>
                            </div>
