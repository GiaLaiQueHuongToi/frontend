...
async function generateImageWithGemini(
  ...
  console.log(`Generating image for segment ${segmentId} with prompt: ${prompt.substring(0, 50)}...`);
  ...
  try {
    ...
    const imageSources = [
      ...
      async () => {
        ...
        const seed = segmentId * 1000 + Math.floor(Math.random() * 1000);
        ...
        console.log(`Pollinations URL for segment ${segmentId}: ${url}`);
        console.log(`Clean prompt: "${cleanPrompt.substring(0, 50)}..."`);
        return url;
      },
      ...
      async () => {
        const themes = ['business', 'technology', 'nature', 'people', 'abstract'];
        ...
        console.log(`Themed Picsum URL for segment ${segmentId}: ${url}`);
        return url;
      },
      ...
      async () => {
        ...
        console.log(`Styled placeholder URL for segment ${segmentId}: ${url}`);
        return url;
      }
    ];
    ...
    for (let i = 0; i < imageSources.length; i++) {
      try {
        ...
        if (testResult) {
          console.log(`✓ Image source ${i + 1} successful for segment ${segmentId}: ${imageUrl}`);
          return imageUrl;
        }
      } catch (error) {
        console.warn(`✗ Image source ${i + 1} failed for segment ${segmentId}:`, error);
        continue;
      }
    }
    ...
    console.log(`Using final fallback for segment ${segmentId}: ${fallbackUrl}`);
    return fallbackUrl;
...
function createReliablePlaceholder(segmentId: number, videoContext: VideoContext): string {
...
function getStyleColors(style: string) {
  ...
  return styleColors[style as keyof typeof styleColors] || styleColors.default;
}
...
function createStyledPlaceholder(
  ...
  const colors = styleColors[style as keyof typeof styleColors] || styleColors.default;
...
`;
      ...
      const imageUrl = await generateImageWithGemini(imagePrompt, scriptSegment.id, videoContext);
  ...
  generateImagesForScript: async (
    ...
    try {
      console.log(`Generating ${scriptSegments.length} images with Gemini AI for "${videoContext.topic}"`);
      ...
      const imagePromises = scriptSegments.map(async (segment, index) => {
        ...
        const { imageUrl, imagePrompt } = await geminiService.generateImage(segment, videoContext);
