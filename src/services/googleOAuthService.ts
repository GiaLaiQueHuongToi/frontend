...
export const googleOAuthService = {
  ...
  getAuthUrl: (): string => {
    if (!process.env.NEXT_PUBLIC_AUTH_GOOGLE_LINK) {
      console.error('Google OAuth link not configured in environment variables');
      return '';
    }
  ...
  exchangeCodeForTokens: async (code: string): Promise<GoogleTokenResponse> => {
    try {
      ...
      params.append('redirect_uri', 'http://localhost:3000/auth/grantcode');
  ...
  storeTokens: async (tokenData: GoogleTokenResponse): Promise<void> => {
    try {
      ...
      const response = await Axios.post('/auth/store-token', storeRequest);
